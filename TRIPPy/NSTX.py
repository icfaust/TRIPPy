...
def diode1(temp,place=(2.0,0,-4e-1), angle=(0,scipy.pi/2,0)):
    ...
    offset = geometry.Origin((0,0.,-2.15e-2),aperature,vec=Vec)
...
def diode3(temp,place=(.8,0,1.8), angle=(0,scipy.pi/2,0)):
    ...
    aperature = surface.Rect((0.,0.,0.),pos,area,vec=Vec)
    Vec = [geometry.Vecx((0.,1.,0.)),geometry.Vecx((0.,0.,1.))]
    offset = geometry.Origin((0,0.,-1.75e-2),aperature,vec=Vec)
    diodes = AXUV.AXUV20(offset)
    for i in diodes:
        i.redefine(temp)
    diodes.append(aperature)
...
def diode7(temp,place=(1.5,0,1.3), angle=(0,scipy.pi/2,0)):
    ...
    aperature = surface.Rect((0.,0.,0.),pos,area,vec=Vec)
    Vec = [geometry.Vecx((0.,1.,0.)),geometry.Vecx((0.,0.,1.))]
    offset = geometry.Origin((0,0.,-4e-2),aperature,vec=Vec)
    diodes = AXUV.AXUV20(offset)
    for i in diodes:
        i.redefine(temp)
    diodes.append(aperature)
